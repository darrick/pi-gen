...
if [ -n "$XRDP_SESSION" -a -n "$XRDP_SOCKET_PATH" ]; then
    ...
    if pactl load-module module-jack-source connect=0; then
        echo "- pulseaudio jack-source loaded"
        if pacmd load-module module-loopback source=jack_in sink=xrdp-sink; then
            echo "- pulseaudio loopback loaded"
        else
            echo "? Can't load pulseaudio loopback"
        fi
    else
        echo "? Can't load pulseaudio jack-source"
        status=1
    fi
    /usr/bin/jack_connect rivendell_0:playout_0L "PulseAudio JACK Source:front-left"
    /usr/bin/jack_connect rivendell_0:playout_0R "PulseAudio JACK Source:front-right"
